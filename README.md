...
- However, one of the main ethical issues we felt we would have to contend with is the "danger of appropriating someone else's story if they are not given control at all moments" (Dush, 2012, p.631). This entails that, as our platform is acting as a sponsor for tenants' personal stories, the platform which we create for story submissions will ultimately dictate the form, content, and parameters of the stories submitted. This idea also encapsulates McLuhan's cornerstone claim that the 'medium [of the technological platform is [penultimately] the message' (McLuhan, 2001)​, that is, the way in which we present the stories will determine and potentially restrict how they are conveyed to other users. To somewhat overcome this ethical and technical dilemma, we decided to adopt zines as a second framing device (see '2. Zine Culture').
<br>
...
- Conceptually, this is what we first envisioned the tenant submissions could be: a 'pop-up' book when clicked on (from on the Tenant Tales map). This idea did not manifest visually as a book format in our end project (as depicted in this prototype) as we found it more important to devote our time to the overall functioning of the map. However, the idea tenant submissions as 'pop-ups' of submissions, but in a simpler format, carried through to our final prototype.
<br>
...
For this iteration, we made some small tweaks in the design of the app / zine to professionalize the looks. Such as adding gradients to the background. and icons to the menu buttons for easier understanding. We've also added a more final representation of what the map would look like, with poems, and artworks that groups together as a collage which users can scroll. This was then merged with the Tenant Handbook and Tenant Roundup and connected via the prototype feature to replicate the full function of the app/ digital zine conceptually. Conceptually the submission from tenants will be done through the + icon where they are given the choice to submit text or any file format. The validity of this submission will be first regulated by the login system put in place at the start of our app, and the content will be checked either manually, or via javascript which will use sentiment analysis for text to flag any innapropriate content.
<br>
...
there are two data protection regulations in total, the first one is GDPR, so called General Data Protection Regulation, which is enforced by EU parliament and Council. it is legally binding across the EU and fully implementable in EU countries. Although Great Britain is no longer a part of EU, it does appied to the UK and it would be good practice to still consider abiding such laws. For example, we followed the GDPR to design the personal data processing to be mankind. Also, the submissions by the users would be anonymised with pseudonyms and data input on our app held with hignest confidentiality, in another way, submissions to the app would have to be read and checked by us but not discussed further. Besides, the confidentiality agreement will be signed and data will be stored exclusively an securely on our system. Thirdly, we will not be processing much data, and certainly not for economic means, but in the consent form, we would state transparently how user data will be collected, stored, and used on and by our app. 
<br>

the second one is Data Protection Act(2018), liekwise, the protection of personal data is clear, this is similar to GDPR, it stipulates the requirment that "personal data need to be processed lawfully and fairly", based on data subject's consent. This Data Protection mentioned that the inaccurate personal data must be rectified. For example, some key words: names, numbers, location data, etc. Moreover, processing of personal data that is necessary for the performance of a task carried out in the public interest or in the exercise of the controller' official authority includes the processing of personal data that is necessary for: "the administration of justice". "the exercis of a function conferred on a person by enactment or rule of law". 
...
Breen, L. and Eilstrup-Sangiovanni, M. (2023). 'Issue-adoption and campaign structure in transnational advocacy campaigns: a longitudinal network analysis', *European Journal of International Relations*, 0(0), pp. 1-31. Available at: https://doi.org/10.1177/13540661231158553 
<br> 
...
Power, E. R. (2017) 'Renting with pets: a pathway to housing insecurity?', *Housing Studies, 32(3), pp. 336-360. Available at: https://doi.org/10.1080/02673037.2016.1210095
<br>
...
Soiata, A. M. and McKee, K. (2019) 'Assembling a 'kind of' home in the UK private renting sector', *Geoforum*, 103, pp. 148-157. Available at: https://doi.org/10.1016/j.geoforum.2019.04.018
<br>
